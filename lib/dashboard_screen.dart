        ...
        return SizedBox(
          ...
          child: FutureBuilder<String>(
            ...
            builder: (context, snapshot) {
                    ...
                    if (sub is Map && sub['timers'] is List) {
                      for (final t in sub['timers']) {
                        if (t['start'] != null && t['end'] != null) {
                          ...
                          if (start != null && end != null && end.month == now.month && end.year == now.year) {
                            final seconds = end.difference(start).inSeconds;
                            secondsPerDay[end.day] = (secondsPerDay[end.day] ?? 0) + seconds;
                          }
                        }
                      }
                    }
              ...
              return LineChart(
                LineChartData(
                  gridData: FlGridData(show: true, drawVerticalLine: true, drawHorizontalLine: true, horizontalInterval: 20, getDrawingHorizontalLine: (v) => FlLine(color: Colors.grey[200], strokeWidth: 1)),
                  titlesData: FlTitlesData(
                    leftTitles: AxisTitles(
                      sideTitles: SideTitles(showTitles: true, reservedSize: 32, interval: 5, getTitlesWidget: (v, meta) {
                        return Text('${v.toInt()} min', style: const TextStyle(fontSize: 11, color: Colors.grey));
                      }),
                    ),
                    bottomTitles: AxisTitles(
                      sideTitles: SideTitles(showTitles: true, reservedSize:28, interval: 5, getTitlesWidget: (v, meta) {
                        if (v % 5 == 0) {
                          return Text('${v.toInt()}', style: const TextStyle(fontSize: 11, color: Colors.grey));
                        }
                        return Container();
                      }),
                    ),
                  ...
                  lineBarsData: [
                    LineChartBarData(
                      ...
                      belowBarData: BarAreaData(show: true, color: Colors.deepPurple.withOpacity(0.18)),
                    ),
                  ],
                  minY: 0,
                  maxY: maxY,
  ...
  Future<void> _loadStats() async {
    ...
    for (final topic in data) {
      ...
      for (final sub in subItems) {
        ...
        final timers = sub is Map && sub['timers'] is List ? sub['timers'] as List : [];
    ...
    setState(() {
      ...
      avgTimePerTask = tasksWithTime > 0 ? Duration(seconds: totalSpent.inSeconds ~/ tasksWithTime) : Duration.zero;
                  ...
                  builder: (context, constraints) {
                    if (constraints.maxWidth < 900) {
                      return Column(
                        children: [
                          SizedBox(
                            ...
                            child: _modernStatCard('Total Tasks', totalTasks, Icons.list_alt, Colors.deepPurple),
                          ),
                          const SizedBox(height: 12),
                          SizedBox(
                            width: double.infinity,
                            child: _modernStatCard('Completed', completedTasks, Icons.check_circle, Colors.teal),
                          ),
                          const SizedBox(height: 12),
                          SizedBox(
                            width: double.infinity,
                            child: _modernStatCard('In Progress', inProgressTasks, Icons.timelapse, Colors.orange),
                          ),
                          const SizedBox(height: 12),
                          SizedBox(
                            width: double.infinity,
                            child: _modernStatCard('Not Started', notStartedTasks, Icons.hourglass_empty, Colors.redAccent),
                          ),
                        ],
                      );
                    } else {
                      return Row(
                        ...
                        children: [
                          Expanded(child: _modernStatCard('Total Tasks', totalTasks, Icons.list_alt, Colors.deepPurple)),
                          Expanded(child: _modernStatCard('Completed', completedTasks, Icons.check_circle, Colors.teal)),
                          Expanded(child: _modernStatCard('In Progress', inProgressTasks, Icons.timelapse, Colors.orange)),
                          Expanded(child: _modernStatCard('Not Started', notStartedTasks, Icons.hourglass_empty, Colors.redAccent)),
                        ],
                      );
                    }
                  },
                              ...
                              child: Padding(
                                ...
                                child: Column(
                                  ...
                                  children: [
                                    Row(
                                      children: [
                                        const Text('Status Mensal', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 18)),
                                        const Spacer(),
                                        Text('Este mês', style: TextStyle(color: Colors.blue, fontWeight: FontWeight.bold)),
                                        const SizedBox(width: 8),
                                        Text(completedThisMonth.toString(), style: TextStyle(color: Colors.grey)),
                                      ],
                                    ),
                                    const SizedBox(height: 12),
                                    SizedBox(
                            ...
                            Card(
                              ...
                              child: Padding(
                                ...
                                child: Column(
                                  ...
                                  children: [
                                    const Text('Tasks Available', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 18)),
                                    const SizedBox(height: 12),
                                    SizedBox(
                                      height: 120,
                                      child: Stack(
                                          ...
                                          Column(
                                            ...
                                            children: [
                                              Text(
                                                ...
                                                style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 32, color: Colors.teal),
                                              ),
                                            ],
                                          ),
                                        ],
                  ...
                  child: Padding(
                    ...
                    child: Column(
                      ...
                      children: [
                        Row(
                          children: [
                            const Text('Gestão do Tempo', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 18)),
                            const Spacer(),
                            Container(
                              ...
                              child: Row(
                                children: [
                                  ...
                                  Text(_formatDuration(totalTimeSpent), style: const TextStyle(fontWeight: FontWeight.bold, color: Colors.blue)),
                                ],
                              ),
                            ),
                          ],
                        ...
                        Text('Tempo médio por tarefa: ${_formatDuration(avgTimePerTask)}', style: const TextStyle(color: Colors.grey)),
                        const SizedBox(height: 16),
                        SizedBox(
                          height: 180,
                          child: _timePerformanceChart(),
                        ),
                      ],
                    ),
                  ),
            ...
            children: [
              ...
              Expanded(
                child: Column(
                  ...
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: [
                    Center(child: Text(value.toString(), style: TextStyle(fontWeight: FontWeight.bold, fontSize: 22, color: color))),
                    Center(child: Text(label, style: TextStyle(color: Colors.grey[700], fontSize: 15))),
                  ],
                ),
              ),
            ],
            ...
            for (final sub in subItems) {
              if (sub is Map && sub['timers'] is List) {
                for (final t in sub['timers']) {
                  if (t['end'] != null) {
                    final end = DateTime.tryParse(t['end'].toString());
                    if (end != null && end.month == now.month && end.year == now.year) {
                      completedPerDay[end.day] = (completedPerDay[end.day] ?? 0) + 1;
                    }
                  }
                }
              }
